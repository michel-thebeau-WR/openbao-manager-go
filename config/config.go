...
func (configInstance MonitorConfig) NewOpenbaoConfig(dnshost string) (*openbao.Config, error) {
	slog.Info(fmt.Sprintf("Setting up api access config for host %v", dnshost))
	...
	slog.Info("Openbao api access config setup complete.")
...
func (configInstance MonitorConfig) SetupClient(dnshost string) (*openbao.Client, error) {
	slog.Info(fmt.Sprintf("Setting up client for host %v", dnshost))
	...
	slog.Info("Client setup complete.")
